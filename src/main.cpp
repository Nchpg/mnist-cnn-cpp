...
static void usage(const char *program) {
    std::cerr << "usage:\n"
              ...
              << "  " << program << " train <mnist.csv> <arch.json> <model_out> [limit=... epochs=... lr=...]\n"
              << "  " << program << " resume <mnist.csv> <model_in> <model_out> [limit=... epochs=... lr=...]\n"
...
static void parse_hp_overrides(int start_idx, int argc, char** argv, Hyperparameters& hp, size_t& epochs, size_t& limit) {
    for (int i = start_idx; i < argc; ++i) {
        ...
        if (key == "lr") hp.learning_rate = std::stof(value);
        else if (key == "batch_size") hp.batch_size = std::stoul(value);
        else if (key == "beta1") hp.beta1 = std::stof(value);
        else if (key == "beta2") hp.beta2 = std::stof(value);
        else if (key == "epsilon") hp.epsilon = std::stof(value);
        else if (key == "optimizer") hp.optimizer_type = (value == "SGD" ? OptimizerType::SGD : OptimizerType::Adam);
        else if (key == "epochs") epochs = std::stoul(value);
        else if (key == "limit") limit = std::stoul(value);
    }
}
...
int main(int argc, char **argv) {
    ...
    if (command == "train") {
        ...
        for (int i = 4; i < argc; ++i) {
            std::string arg = argv[i];
            if (arg.substr(0, 6) == "limit=") limit = std::stoul(arg.substr(6));
        }
        ...
        for (int i = 4; i < argc; ++i) {
            std::string arg = argv[i];
            if (arg.substr(0, 6) == "index=") index = std::stoul(arg.substr(6));
        }
