...
static bool parse_pixels(const std::string &body, Matrix &image)
{
    ...
    for (size_t row = 0; row < IMG_HEIGHT; row++)
    {
        ...
        {
            scalar_t raw =
                web_pixels[col * IMG_HEIGHT + row] / NORMALIZE_DIVISOR;
            image(row * IMG_HEIGHT + col, 0) = MnistDataset::normalize()
                ? (raw - MnistDataset::mean()) / MnistDataset::std()
                : raw;
        }
    }
    return true;
