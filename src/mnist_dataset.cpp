...
MnistDataset MnistDataset::load_bin(const std::string& path, size_t limit) {
    ...
    if (!file.is_open()) throw std::runtime_error("Failed to open binary file: " + path);
    ...
    while (limit == 0 || dataset.count_ < limit) {
        if (!file.read(reinterpret_cast<char*>(&label), 1)) break;
    ...
    if (dataset.count_ == 0) throw std::runtime_error("Binary file is empty or invalid: " + path);
...
MnistDataset MnistDataset::load_csv(const std::string& path, size_t limit) {
    ...
    std::string line;
    if (!std::getline(file, line)) throw std::runtime_error("File is empty: " + path);
    ...
    if (!line_looks_like_header(line)) parse_line(line);
    ...
    if (dataset.count_ == 0) throw std::runtime_error("File contains no MNIST rows: " + path);
    dataset.gen_ = std::mt19937(42);
    dataset.shuffle_indices();
    return dataset;
}
...
void MnistDataset::get_batch_images(size_t start_idx, size_t batch_size, Matrix& out_batch) const {
    if (start_idx + batch_size > count_) batch_size = count_ - start_idx;
