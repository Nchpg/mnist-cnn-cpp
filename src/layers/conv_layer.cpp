...
ConvLayer::ConvLayer(size_t input_rows, size_t input_cols,
                     size_t input_channels, size_t kernel_size,
                     size_t filter_count, std::mt19937 &gen,
                     scalar_t /*weight_scale*/)
...
static inline size_t get_im2col_col(size_t b, size_t y, size_t x, size_t out_h,
                                    size_t out_w)
{
                ...
                for (size_t kx = 0; kx < k_s; ++kx)
                {
                    ...
                    for (size_t y = 0; y < out_h; ++y)
                    {
                        for (size_t x = 0; x < out_w; ++x)
                        {
                            ...
                            out_col(row, col) = TensorBatch::read_mat(
                                input, b, c, y + ky, x + kx, in_h, in_w);
                        }
                    }
                }
            }
...
const Matrix &ConvLayer::forward(const Matrix &input)
{
    ...
    if (output_.batch_size() != b_size || im2col_buffer_.rows() == 0)
    {
        ...
        grad_input_.reshape4D(grad_input_.channels(), grad_input_.height(),
                              grad_input_.width(), b_size);

        size_t weights_per_filter =
            grad_input_.channels() * kernel_size_ * kernel_size_;
...
const Matrix &ConvLayer::backward(const Matrix &gradient)
{
    ...
    if (filters_grad_matrix_.rows() != f_count
        || filters_grad_matrix_.cols() != weights_per_filter)
    {
    ...
    for (size_t b = 0; b < b_size; ++b)
    {
        for (size_t f = 0; f < f_count; ++f)
        {
            for (size_t y = 0; y < out_h; ++y)
            {
                ...
                {
                    size_t col = get_im2col_col(b, y, x, out_h, out_w);
                    gemm_out_(f, col) = TensorBatch::read_mat(gradient, b, f, y,
                                                              x, out_h, out_w);
                }
            }
        }
...
{
    return { { &filters_matrix_, &filters_grad_matrix_ },
...
void ConvLayer::save(std::ostream &os) const
{
    ...
    os.write(reinterpret_cast<const char *>(&kernel_size_),
             sizeof(kernel_size_));
    biases_.save(os);
...
{
    return { { "type", "Conv" },
             { "filters", filters_matrix_.rows() },
             { "kernel_size", kernel_size_ } };
}

Shape3D ConvLayer::get_output_shape(const Shape3D &input_shape) const
{
    return { filters_matrix_.rows(), input_shape.height - kernel_size_ + 1,
             input_shape.width - kernel_size_ + 1 };
}
